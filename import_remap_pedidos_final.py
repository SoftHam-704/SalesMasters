...
def clean_date(val):
    ...
    # Filter out weird dates like 1899 or 0000
    if '1899' in s or '0000' in s: return None
    
    # Try parsing 'DD.MM.YYYY'
    try:
        if '.' in s:
            day, month, year = s.split('.')
            if len(year) == 4 and year != '1899':
...
def clean_value(col, val, max_len=None):
    ...
    if 'data' in col or 'emissao' in col or 'dt' in col:
        res = clean_date(val)
        return res # Return None if invalid
...
def import_pedidos():
    print(f"--- Starting Import V5: {TABLE} (Batch: {BATCH_SIZE}) ---")
    ...
    try:
        ...
        # Keep truncate? Or append since 26k inserted? 
        # Better truncate to ensure clean state and no duplicates if partially inserted
        ...
        for record in data:
            ...
            for col in target_cols:
                ...
                max_len = col_specs.get(col, {}).get('max_len')
                
                # Fix ped_tabela
                if col == 'ped_tabela':
                    if not val or str(val).strip() == '': val = 'ND'
