...
const ClientForm = ({ data, onClose, onSave, open, onOpenChange }) => {
    ...
    const fetchContacts = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:3005/api/clients/${data.cli_codigo}/contacts`);
            if (res.ok) {
    ...
    const fetchIndustries = async () => {
        ...
        try {
            const res = await fetch(`http://localhost:3005/api/clients/${data.cli_codigo}/industries`);
            if (res.ok) {
                const json = await res.json();
    ...
    const handleDeleteContact = async (contact) => {
        ...
        try {
            const res = await fetch(`http://localhost:3005/api/clients/${data.cli_codigo}/contacts/${contact.ani_lancto}`, {
                method: 'DELETE'
            });
            if (res.ok) {
    ...
    const handleDeleteIndustry = async (ind) => {
        ...
        try {
            const res = await fetch(`http://localhost:3005/api/clients/${data.cli_codigo}/industries/${ind.cli_lancamento}`, {
                method: 'DELETE'
            });
            if (res.ok) {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (citySearch && citySearch.length > 2) {
                fetch(`http://localhost:3005/api/aux/cidades?search=${citySearch}`)
                    .then(res => res.json())
                    .then(data => setCityOptions(data))
                    .catch(e => console.error(e));
            } else if (!citySearch) {
                // Initial load (top 10) or reset
                fetch(`http://localhost:3005/api/aux/cidades`)
                    .then(res => res.json())
                    .then(data => setCityOptions(data))
                    .catch(e => console.error(e));
            }
    ...
    useEffect(() => {
        if (formData.cli_idcidade) {
            fetch(`http://localhost:3005/api/aux/cidades?id=${formData.cli_idcidade}`)
                .then(res => res.json())
                .then(data => {
    ...
    const loadAuxData = async () => {
        ...
        try {
            const [areasRes, sellersRes, regionsRes] = await Promise.all([
                fetch('http://localhost:3005/api/aux/areas'),
                fetch('http://localhost:3005/api/aux/vendedores'),
