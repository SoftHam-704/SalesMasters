...
def analyze_industry_growth(ano: int, metric: str = 'valor', limit: int = 15):
    ...
    try:
        ...
        # Ensure columns exist even if query returns partial schema (unlikely but safe)
        if 'atual' not in df.columns: df['atual'] = 0.0
        if 'anterior' not in df.columns: df['anterior'] = 0.0
