...
def fetch_dashboard_summary(ano: int, mes: str = 'Todos', industria: int = None, startDate: str = None, endDate: str = None):
    """
    Retorna KPIs do dashboard com comparativo M-1 ou A-1.
    ...
    """
    print(f"--- BI: Fetching Dashboard Summary ({ano}/{mes}) range={startDate}:{endDate} industria={industria} ---", flush=True)
    ...
    if startDate and endDate:
        # Range comparison: Range vs Same range in Previous Year
        query = f"""
    ...
    try:
        ...
        if df.empty:
            # Safe defaults if query fails or returns no rows
            default_row = {
                "total_vendido_current": 0.0, "qtd_pedidos_current": 0,
                "clientes_atendidos_current": 0, "quantidade_vendida_current": 0.0,
                "ticket_medio_current": 0.0, "total_vendido_prev": 0.0,
                "qtd_pedidos_prev": 0, "clientes_atendidos_prev": 0,
                "quantidade_vendida_prev": 0.0, "ticket_medio_prev": 0.0
            }
            row = default_row
        else:
            row = df.iloc[0]
        ...
        curr_vendido = float(row.get('total_vendido_current', 0) or 0)
        curr_pedidos = int(row.get('qtd_pedidos_current', 0) or 0)
        curr_clientes = int(row.get('clientes_atendidos_current', 0) or 0)
        curr_quantidade = float(row.get('quantidade_vendida_current', 0) or 0)
        curr_ticket = float(row.get('ticket_medio_current', 0) or 0)
        
        prev_vendido = float(row.get('total_vendido_prev', 0) or 0)
        prev_pedidos = int(row.get('qtd_pedidos_prev', 0) or 0)
        prev_clientes = int(row.get('clientes_atendidos_prev', 0) or 0)
        prev_quantidade = float(row.get('quantidade_vendida_prev', 0) or 0)
        prev_ticket = float(row.get('ticket_medio_prev', 0) or 0)
        ...
        traceback.print_exc()
        default_summary = {
            "total_vendido_current": 0, "qtd_pedidos_current": 0, "clientes_atendidos_current": 0,
            "quantidade_vendida_current": 0, "ticket_medio_current": 0,
            "vendas_percent_change": 0, "pedidos_percent_change": 0,
            "clientes_percent_change": 0, "quantidade_percent_change": 0, "ticket_percent_change": 0
        }
