...
async function cloneSchema() {
    ...
    try {
        ...
        console.log(`⚙️ Executando DDL no banco para schema ${targetSchema}...`);
        await pool.query(sql);
        console.log(`✅ Estrutura clonada com sucesso em ${targetSchema}.`);

        // 5. Clonar dados das tabelas base requeridas
        console.log('📊 Clonando dados de cidades...');
        ...
        const resFunctions = await pool.query(`SELECT count(*) FROM information_schema.routines WHERE routine_schema = '${targetSchema}'`);

        console.log(`📈 Resultados em ${targetSchema}:`);
        console.log(`   - Tabelas: ${resTables.rows[0].count}`);
        console.log(`   - Views:   ${resViews.rows[0].count}`);
        console.log(`   - Funções: ${resFunctions.rows[0].count}`);
