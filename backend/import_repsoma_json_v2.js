...
async function importJson() {
    ...
    try {
        console.log('🚀 Iniciando Importação Repsoma (Sem ON CONFLICT)...');
        ...
        for (let i = 0; i < pedidos.length; i += 500) {
            const chunk = pedidos.slice(i, i + 500);
            const values = [];
            const placeholders = [];
            ...
            const sql = `INSERT INTO ${schema}.pedidos (ped_numero, ped_pedido, ped_tabela, ped_data, ped_industria, ped_cliente, ped_transp, ped_vendedor, ped_situacao, ped_totliq) VALUES ${placeholders.join(',')}`;
            await pool.query(sql, values);
            process.stdout.write(`\rProcessado: ${Math.min(i + 500, pedidos.length)}/${pedidos.length}`);
        }
        console.log('\n✅ Pedidos importados.');
        ...
        for (let i = 0; i < itens.length; i += 500) {
            ...
            const sql = `INSERT INTO ${schema}.itens_ped (ite_lancto, ite_pedido, ite_industria, ite_produto, ite_nomeprod, ite_data, ite_quant, ite_puni, ite_puniliq, ite_totliquido) VALUES ${placeholders.join(',')}`;
            await pool.query(sql, values);
            process.stdout.write(`\rProcessado: ${Math.min(i + 500, itens.length)}/${itens.length}`);
        }
        console.log('\n✅ Itens importados.');
