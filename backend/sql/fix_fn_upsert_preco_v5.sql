-- Fix: Atualizar fn_upsert_preco para 13 parâmetros (incluindo p_prepeso)
...
DECLARE 
    r RECORD;
BEGIN
    -- Drop all existing versions of fn_upsert_preco in ALL schemas
    FOR r IN (
    ...
    LOOP
        BEGIN
            ...
            RAISE NOTICE 'Dropped: %.%', r.nspname, r.proname;
...
-- Criar versão MASTER no public com 13 parâmetros
CREATE OR REPLACE FUNCTION public.fn_upsert_preco(
    ...
    p_prepeso DOUBLE PRECISION DEFAULT 0  -- NOVO PARÂMETRO
)
RETURNS VOID
LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    LOOP
        EXECUTE format('
            ...
            END;
            $func$;', schema_name);
        RAISE NOTICE 'Created fn_upsert_preco in schema: %', schema_name;
    END LOOP;
END $$;
