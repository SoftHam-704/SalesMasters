-- Correção da função get_dashboard_metrics
-- 1. Total Vendido: Soma de itens_ped.ite_totliquido (mais confiável que ped_totliq)
-- 2. Status: Filtrar apenas P (Pendente) e F (Faturado/Finalizado)

DO $$ 
DECLARE 
    r RECORD;
BEGIN
    -- Drop all existing versions of get_dashboard_metrics in ALL schemas
    FOR r IN (
...
BEGIN
    ...
    -- TOTAL VENDIDO: Soma de ite_totliquido (via JOIN com pedidos)
    -- QUANTIDADE VENDIDA: Soma de ite_quant  
    -- Filtro: ped_situacao IN ('P', 'F') - Pendente e Faturado
    SELECT 
        COALESCE(SUM(i.ite_totliquido), 0),
        COALESCE(SUM(i.ite_quant), 0)
    INTO v_total_atual, v_qtd_atual
    FROM itens_ped i
    ...
    -- CLIENTES ATENDIDOS e TOTAL PEDIDOS (da tabela pedidos)
    SELECT 
        COUNT(DISTINCT p.ped_cliente), 
        COUNT(DISTINCT p.ped_pedido)
    INTO v_cli_atual, v_ped_atual
    FROM pedidos p
    WHERE p.ped_data BETWEEN v_inicio_atual AND v_fim_atual 
      AND p.ped_situacao IN ('P', 'F')
      AND (p_industria IS NULL OR p_industria = 0 OR p.ped_industria = p_industria);
    ...
    -- =====================================================
    -- PREVIOUS PERIOD METRICS (same logic, previous year)
    -- =====================================================
    
    SELECT 
        COALESCE(SUM(i.ite_totliquido), 0),
        COALESCE(SUM(i.ite_quant), 0)
    INTO v_total_anterior, v_qtd_anterior
    FROM itens_ped i
    JOIN pedidos p ON p.ped_pedido = i.ite_pedido
    ...
    SELECT 
        COUNT(DISTINCT p.ped_cliente), 
        COUNT(DISTINCT p.ped_pedido)
    INTO v_cli_anterior, v_ped_anterior
    FROM pedidos p
    WHERE p.ped_data BETWEEN v_inicio_anterior AND v_fim_anterior 
      AND p.ped_situacao IN ('P', 'F')
      AND (p_industria IS NULL OR p_industria = 0 OR p.ped_industria = p_industria);
    ...
    -- =====================================================
    -- RETURN RESULTS WITH PERCENT CHANGES
    -- =====================================================
    RETURN QUERY SELECT 
        COALESCE(v_total_atual, 0.0),
        CASE WHEN COALESCE(v_total_anterior, 0.0) > 0 THEN ((v_total_atual - v_total_anterior) / v_total_anterior) * 100.0 ELSE 0.0 END,
        COALESCE(v_qtd_atual, 0.0),
        CASE WHEN COALESCE(v_qtd_anterior, 0.0) > 0 THEN ((v_qtd_atual - v_qtd_anterior) / v_qtd_anterior) * 100.0 ELSE 0.0 END,
        COALESCE(v_cli_atual, 0::bigint),
        CASE WHEN COALESCE(v_cli_anterior, 0.0) > 0 THEN ((v_cli_atual - v_cli_anterior)::DOUBLE PRECISION / v_cli_anterior) * 100.0 ELSE 0.0 END,
        COALESCE(v_ped_atual, 0::bigint),
        CASE WHEN COALESCE(v_ped_anterior, 0.0) > 0 THEN ((v_ped_atual - v_ped_anterior)::DOUBLE PRECISION / v_ped_anterior) * 100.0 ELSE 0.0 END;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    FOR schema_name IN SELECT nspname FROM pg_namespace WHERE nspname NOT IN ('information_schema', 'pg_catalog', 'pg_toast') LOOP
        ...
        EXECUTE format('
            CREATE OR REPLACE FUNCTION %1$I.get_dashboard_metrics(
                p_ano INTEGER, 
                p_mes INTEGER DEFAULT NULL,
                p_industria INTEGER DEFAULT NULL
            )
