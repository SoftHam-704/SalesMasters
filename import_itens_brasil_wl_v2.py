...
def import_sheet(cur, file_path, table_name):
    ...
    # Mapeamento e fallback para campos obrigatórios
    # Se ite_idproduto for nulo, vamos preencher com um valor padrão (ex: 0) ou tentar achar no banco 
    # se fosse uma importação smart, mas aqui vamos apenas garantir que não seja nulo se o banco exigir.
    
    valid_cols = [c for c in df.columns if c in db_cols_meta]
    ...
    # Lista de colunas que não podem ser nulas (baseado no erro ite_idproduto)
    not_null_cols = [col for col, meta in db_cols_meta.items() if not meta['nullable']]
    ...
    for _, row in df.iterrows():
        try:
            ...
            errors += 1
            if errors <= 10:
                print(f"   ❌ Erro no registro {count+errors}: {e}")
    
    print(f"✅ FINALIZADO {table_name}: Sucesso: {count} | Falhas: {errors}")

def run():
    print(f"🚀 Iniciando importação EXCLUSIVA de ITENS_PED no schema '{SCHEMA}' (com correção de nulos)...")
