...
const OrderItemEntry = ({
    ...
    const getFilteredProducts = () => {
        ...
        return products.filter(p => {
            ...
            if (searchMode === 'C') return matchesCode || matchesNorm;
            return matchesCode || matchesNorm || matchesDesc;
        });
    };
                ...
                <div className="bg-white border border-emerald-100 rounded-xl p-4 shadow-sm flex flex-col gap-3">
                    <div className="flex gap-2">
                        <div className="w-32">
                            ...
                            <Input
                                ...
                                onFocus={handleFocus}
                                className={cn(inputClasses, "bg-amber-50 border-amber-200 font-bold text-black h-9 text-sm")}
                        ...
                        <div className="w-32">
                            ...
                            <Input
                                ...
                                className={cn(inputClasses, "h-9 text-sm", !enableComplement && "bg-slate-100 text-slate-400")}
                            />
                        </div>
                        <div className="flex-1">
                            ...
                            <Input
                                ...
                                readOnly
                                className={cn(inputClasses, "bg-slate-50 border-slate-200 truncate h-9 text-sm")}
                            />
                        </div>
                    </div>
                        ...
                        {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(num => (
                            <div key={num} className="flex-1 min-w-[50px]">
                                ...
                                <Input
                                    ...
                                    className={cn("h-7 text-[11px] text-center p-0 rounded-md border-emerald-100 focus:border-emerald-400 bg-emerald-50/20 text-emerald-700 font-bold", !enableDiscounts && "bg-slate-100 text-slate-400")}
                                />
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="flex gap-2 items-end flex-wrap">
                        <div className="w-16">
                            ...
                            <Input
                                ...
                                className={cn(inputClasses, "bg-emerald-50 text-center font-bold text-teal-700 border-emerald-300 h-9 text-sm", !enableAddPercent && "bg-slate-100 text-slate-400")}
                            />
                        </div>
                        <div className="w-24">
                            ...
                            <Input
                                ...
                                className={cn(inputClasses, "text-center font-black h-10 text-base border-emerald-400 bg-white")}
                            />
                        </div>
                        <div className="w-20">
                    ...
                    <div className="flex gap-4 justify-end pt-2">
                        <Button
                            ...
                            className="bg-white border-2 border-emerald-200 text-emerald-700 hover:bg-emerald-50 h-10 px-8 font-black shadow-sm flex items-center gap-2 uppercase tracking-widest active:scale-95 transition-all"
                        >
                            <Check className="h-5 w-5" /> Salvar (ENTER)
                        </Button>
                        <Button
                            onClick={handleFinalizeItems}
                            onKeyDown={(e) => { if (e.key === 'Enter') handleFinalizeItems(); }}
                            disabled={syncing || orderItems.length === 0}
                            className="bg-emerald-600 hover:bg-emerald-700 text-white h-10 px-8 font-black shadow-md flex items-center gap-2 uppercase tracking-widest active:scale-95 transition-all min-w-[200px]"
                        >
                            {syncing ? <RefreshCw className="h-5 w-5 animate-spin" /> : <Check className="h-5 w-5" />}
                            Finalizar Itens
                        </Button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-auto">
                    ...
                    {rightPanelTab === 'tabela' && (
                        <table className="w-full text-[13px]">
                            <thead className="bg-[#f8fafc] text-slate-700 sticky top-0 uppercase font-black border-b-2 border-emerald-200 z-10">
                                <tr className="h-12">
                                    <th className="px-3 border-b text-left">Código</th>
                                    <th className="px-3 border-b text-left">Conv.</th>
                                    <th className="px-3 border-b text-left">Descrição</th>
                                    <th className="px-3 border-b text-right">Bruto</th>
                                    <th className="px-3 border-b text-right text-orange-600">Promo</th>
                                    <th className="px-3 border-b text-right text-blue-600">Especial</th>
                                    <th className="px-3 border-b text-center">Mult</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {loadingProducts ? (
                                    <tr><td colSpan="5" className="p-10 text-center text-emerald-400 italic font-medium">Carregando produtos...</td></tr>
                                ) : products.length === 0 ? (
                                    <tr><td colSpan="7" className="p-10 text-center text-emerald-300 italic font-medium">Selecione uma tabela de preços</td></tr>
                                ) : (
                                    getFilteredProducts().map((product, idx) => (
                                        <tr
                                            ...
                                            className={cn(
                                                "cursor-pointer transition-all h-12 group",
                                                idx === selectedProductIndex
                                                    ? "bg-emerald-600 text-white ring-2 ring-emerald-700 ring-inset scale-[1.01] z-10 shadow-lg"
                                                    : "hover:bg-emerald-50 text-slate-800"
                                            )}
                                        >
                                            <td className={cn("px-3 font-black text-sm", idx === selectedProductIndex ? "text-white" : "text-emerald-800 uppercase")}>{product.pro_codprod}</td>
                                            <td className={cn("px-3 font-mono text-[11px]", idx === selectedProductIndex ? "text-emerald-100" : "text-slate-400")}>{product.pro_codigonormalizado}</td>
                                            <td className={cn("px-3 font-bold truncate max-w-[200px] text-[13px]", idx === selectedProductIndex ? "text-white" : "text-slate-900")}>{product.pro_nome}</td>
                                            <td className={cn("px-3 text-right font-black text-sm", idx === selectedProductIndex ? "text-white" : "text-emerald-600")}>
                                                {parseFloat(product.itab_precobruto || 0).toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}
                                            </td>
                                            <td className={cn("px-3 text-right font-black text-sm", idx === selectedProductIndex ? "bg-emerald-700" : (parseFloat(product.itab_precopromo || 0) > 0 ? "text-orange-600 bg-orange-50/80" : "text-slate-300"))}>
                                            ...
                                            <td className={cn("px-3 text-right font-black text-sm", idx === selectedProductIndex ? "bg-emerald-700" : (parseFloat(product.itab_precoespecial || 0) > 0 ? "text-blue-600 bg-blue-50/80" : "text-slate-300"))}>
                                            ...
                                            <td className={cn("px-3 text-center font-black", idx === selectedProductIndex ? "text-emerald-100" : "text-slate-400")}>{product.pro_mult || '---'}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
