...
class AdvancedAnalyzer:
    ...
    def detect_anomalies(self):
        ...
        WHERE ca.desvio_padrao > 0 
          ...
          AND cs.semana >= CURRENT_DATE - INTERVAL '4 weeks'
    ...
    def find_product_correlations(self):
    ...
    def detect_lost_opportunities(self):
    ...
    def predict_churn_with_context(self):
...
def get_advanced_insights(ano: int = None, industry_id: int = None):
    ...
    # ==========================================
    cache_key = f"insights_{ano}_{industry_id}"
    cached = get_cached_insights(cache_key)
    if cached:
        return cached
    
    start_time = time.time()
    print(f"[INSIGHTS] Iniciando get_advanced_insights (ano={ano}, ind={industry_id})", flush=True)
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {
                executor.submit(run_query, 'anomalies', analyzer.detect_anomalies): 'anomalies',
                executor.submit(run_query, 'correlations', analyzer.find_product_correlations): 'correlations',
                executor.submit(run_query, 'opportunities', analyzer.detect_lost_opportunities): 'opportunities',
                executor.submit(run_query, 'churn', analyzer.predict_churn_with_context): 'churn',
...
def generate_insights(ano: int, industry_id: int = None):
    ...
    result = get_advanced_insights(ano, industry_id)
