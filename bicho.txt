...
// SERVIR arquivos estáticos (Frontend e Imagens)
...
// Rota Principal: Serve o index.html (React)
...
app.post('/api/config/database/save', async (req, res) => {
    ...
    try {
        ...
        const updateEnv = (key, value) => {
            ...
            return regex.test(envContent) ? envContent.replace(regex, `${key}=${value}`) : `${envContent}\n${key}=${value}`;
        };

        envContent = updateEnv('DB_HOST', host);
        envContent = updateEnv('DB_PORT', port);
        envContent = updateEnv('DB_NAME', database);
        envContent = updateEnv('DB_USER', user);
        envContent = updateEnv('DB_PASS', password);
        envContent = updateEnv('DB_SSL', ssl);
        ...
        if (checkResult.rows.length > 0) {
            await pool.query(`
                UPDATE empresa_status SET
                    ...
                    emp_basedadoslocal = $12, emp_host = $13, emp_porta = $14,
                    emp_username = $15, emp_password = $16, emp_pastabasica = $17,
                    emp_dataatualizacao = CURRENT_TIMESTAMP
                WHERE emp_id = 1
            `, [
                config.situacao || 'A', config.nome, config.endereco, config.bairro,
                config.cidade, config.uf, config.cep, config.cnpj,
                config.inscricao, config.fones, config.logotipo,
                config.baseDadosLocal, config.host, config.porta,
                config.username, config.password, config.pastaBasica
            ]);
        } else {
            await pool.query(`
                INSERT INTO empresa_status (emp_id, emp_nome, emp_cnpj) VALUES (1, $1, $2)
            `, [config.nome, config.cnpj]);
        }
        res.json({ success: true, message: 'Configuração da empresa salva!' });
    } catch (error) {
        res.status(500).json({ success: false, message: `Erro ao salvar: ${error.message}` });
    }
});
...
// Endpoints auxiliares movidos para arquivos externos ou unificados
